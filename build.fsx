...
let clone gitUrl commit name = 
    let target = projectDir name
    Repository.clone rootDir gitUrl target
    Branches.checkoutNewBranch target commit "temp-test"

Target "CloneProjects" (fun _ ->
    for name,gitUrl,commit,_,_,_ in projects do
        clone gitUrl commit name
)

Target "BuildProjects" (fun _ ->
    for name,_,_,command,_,_ in projects do
        let result = 
...
Target "RunPerfTests" (fun _ ->
    ...
    for hash in compilerHashes do
        ...
        for run in 1..runs do
            for name,_,_,_,dir,args in projects do
